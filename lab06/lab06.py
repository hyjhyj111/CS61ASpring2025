class Transaction:
    ...
    def report(self):
        ...
        if self.changed():
            "*** YOUR CODE HERE ***"
            if before < after:
                msg = f'increased {before}->{after}'
            else:
                msg = f'decreased {before}->{after}'
...
class BankAccount:
    ...
    def add(self, before, after):
        if not self.transactions:
            ...
            self.transactions.append(Transaction(self.transactions[-1].id + 1, before, after))
    ...
    def deposit(self, amount):
        ...
        """
        before = self.balance
        after = self.balance = self.balance + amount
